...
if strcmp(normalization, 'Energy')
    pupil.E = pupil.A.*exp(2*pi*1i*(pupil.xx * theta(1) + pupil.yy * theta(2))/sci.f/sci.lambda)/sqrt(pupil.Area); % energy normalized
else
    pupil.E = pupil.A.*exp(2*pi*1i*(pupil.xx * theta(1) + pupil.yy * theta(2))/sci.f/sci.lambda)/pupil.Area; % contrast units
end
